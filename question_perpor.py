...
good_set = set(['于倍', '于分', '过分', '过块', '招人', '过的', '前的', '过倍', '年月', '到分', '在月', '在亿',
                ...
                '于个', '超册', '场月', '于岁', '过岁', '在周', '为且', '招名', '的克', '是份', '是个', '达平', '我年', '是平', '为的', '为是', '在个',
                '过枚', '有个',
                '由个', '中个', '在以', '有个', '有集', '到亿', '过的', '止的', '了册', '超的', '聘个', '于集', '给块', '要元', '在号', '在公', '在百',
                '在元', '米元',
                '道块', '卖元', '要？', '聘名', '到块', '卖块', '过元', '过集', '于枚', '于册', '为人', '是题', '诶号', '了平', '达亿', '有家', '映天',
                '破亿', '破千',
                '在以', '止个', '和的', '有平', '超亿', '于亿', '超个', '超块', '破元', '超平', '近套', '在平', '些年', '和年', '或月', '了平', '破的',
                '超倍', '在亿',
...
bad_set = set(
    ['问下', '查下', '解下', '询下', '第季', '说下', '我下', '这本', '共', '这周', '哪个', '查本', '这个', '知下', '问共', '第第', '第', '第周', '算下',
     '了等', '估下',
...
digit_dict = {'零': 0, '一': 1, '二': 2, '三': 3, '四': 4, '五': 5, '六': 6, '七': 7, '八': 8, '九': 9, '十': 10, '百': 100,
              '千': 1000, '万': 10000,
...
def get_data_and_table():
    ...
    获取所有可用数据和表
    """
    train_data, train_tables = read_data('../datasets/NL2SQL/train/train.json', '../datasets/NL2SQL/train/train.tables.json')
    valid_data, valid_tables = read_data('../datasets/NL2SQL/val/val.json', '../datasets/NL2SQL/val/val.tables.json')
    test_data, test_tables = read_data('../datasets/NL2SQL/test/test.json', '../datasets/NL2SQL/test/test.tables.json')
    all_data = []
...
def trans_question_acc_test():  # 调用trans_question_acc
    """
